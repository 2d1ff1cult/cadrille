...
def run(data_path, log_path, mode, use_text):
    ...
    trainer = Trainer(
        ...
        args=TrainingArguments(
            ...
            eval_steps=2000,  # More frequent evaluation (was 500)
            save_steps=2000,  # More frequent saves (was 500)
            logging_steps=500,  # More frequent logging (was 100)
            
            # MEMORY OPTIMIZATION for RTX 6000 Ada
            gradient_accumulation_steps=accumulation_steps,
            gradient_checkpointing=True,
            dataloader_num_workers=4,  # Increased from 8 for faster data loading
            dataloader_persistent_workers=True,
            dataloader_pin_memory=True,  # Faster CPU->GPU transfer
            
            # MIXED PRECISION - Crucial for speed on Ada architecture
            bf16=True,  # BF16 is optimal for Ada generation
            bf16_full_eval=False,  # Was True, use BF16 for evaluation too
    ...
    # address known issue with python garbage collection
    import gc
    gc.collect()
    torch.cuda.empty_cache()
